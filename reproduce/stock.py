...
t0 = time.time()
merger = StockMerger(models, task='text-classification', base_index=0, passthrough_layers=['classifier.bias', 'classifier.weight'])
pipe.model = merger.merge()
print('Merging completed. Time elapsed:', time.time()-t0)
# Save the merged weights
pipe.model.save_pretrained(f'./artifacts/merged_weights_stock_{DATASET}_{SPLIT}.pt')
